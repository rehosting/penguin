...
# Default passwords
default_auths = [('admin', 'admin'), ('admin', ''), ('', ''), ('user', 'user'), ('user', '')]
...
class TargetInfo(object):
    ...
    def parse_response(self, url, response):
        ...
        if any([url.endswith(x) for x in [".png", ".jpg"]]):
            ...
            with open(f"/tmp/{url.replace('/','_')}_{id(self)}", "wb") as f:
...
class PandaCrawl(PyPlugin):
    ...
    def __init__(self, panda):
        ...
        self.collect_opens = False # When set we record all opens we see during a request
        self.opens = []
        ...
        self.auth = None # can become tuple (user, pw)
    ...
    def ls_filesystem(self, path):
        ...
        if not hasattr(self, 'filesystem_contents'):
            self.filesystem_contents = subprocess.check_output(f"tar -tf '{self.fwdir}/fs.tar'", shell=True).decode().splitlines()

        if not hasattr(self, 'tarbase'):
            self.tarbase = subprocess.check_output(f"tar -tf '{self.fwdir}/fs.tar' | head -n1", shell=True).decode().strip()
    ...
    def find_filesystem(self, filename):
        self.ls_filesystem("/missing") # Ensure we have fs contents
        return [x for x in self.filesystem_contents if x.endswith(filename)]
    
    def get_filesystem(self, path):
        rv = subprocess.check_output(f"tar -xf '{self.fwdir}/fs.tar' -O '{path}", shell=True).decode().strip()
        return rv
    ...
    def check_syscall(self, sysc_string, sysname, context=None):
        ...
        if sysname in ['open', 'openat'] and self.collect_opens:
            self.opens.append(sysc_string)
    ...
    def on_bind(self, proto, guest_ip, guest_port, host_port, procname):
        ignoring = guest_port != 80 or proto != 'tcp'

        self.logger.info(f"Saw bind from {procname} {proto} to {guest_ip}:{guest_port}, mapped to host port {host_port}" + " - Ignoring non tcp::80" if ignoring else "")
    ...
    def crawl_manager(self):
        ...
        sleep(10) # Give VPN and guest server a chance to start up (maybe can shorten or do dynamically?
            ...
            if target.has_next_form():
                ...
                for params in request_params:
                    ...
                    for form_text in target.parse_response(url, response):
                        self.parse_form(target, form_text, url, (procname, proto, host_port, guest_ip, guest_port))
                ...
                # We make a request, if it's a 401 we retry. That's all this loop is
                retry = True 
                bail = False

                while retry:
                    # Crawl it. TODO: support params / non-get methods for non-forms
                    retry = False # Only make a request once, unless it's result means we need to retry
                    try:
    ...
    def find_basic_auth(self, target, url):
        self.logger.error(f"Need basic auth in request to {url}")

        # First, analyze FS to check for known auth files
        htpwd = self.find_filesystem("htpasswd")
        for f in htpwd:
            print(f"TODO: Learn from htpasswd at {f}")
            data = self.get_filesystem(f)
            print(data)
            for line in data.splitlines():
                if ":" in line:
                    user, pw = line.split(":")
                    auths.append((user, pw)) # XXX pw is hashed
                    print(f"FOUND CREDS {user}: hashed pw: {pw}")


        for auth_try in default_auths:
            self.active_request = (target, requests.Request('GET', url))
            try:
            ...
            # Never found a valid auth
            return False

        # Store credentials for future requests
        print("Found auth:", auth_try)
        self.auth = auth_try
        target.session.auth = self.auth


    def parse_form(self, target, bs_form, url, target_key):
        '''
        ...
        if any([x['type'] == "password" for x in form['params'].values()]):
            # Might be a login form! Try to figure it out
            print("POTENTIAL LOGIN FORM:", form)

            # We can only handle a single password field
            users = [(name, x) for (name, x) in form['params'].items() if x['type'] == 'text']
            pws = [(name, x) for (name, x)  in form['params'].items() if x['type'] == 'password']
            if len(pws) != 1 or len (users) != 1:
                raise RuntimeError("Can only handle login with 1 text and 1 pw field")
            (user, pw) = (users[0], pws[0])
            print(f"USER: {user} PW: {pw}")
            if not self.find_login(target, url, abs_act, form, target_key, user, pw):
                self.logger.warning(f"Failed to find login for {url} with {form}")
            # When we find a login, we bail and skip the normal fuzzing
            return
 
        self.logger.info(f"Recording form that {form['method']}s to {abs_act}")
        self.do_form_queue(abs_act, form, target_key)

    def find_login(self, target, base_url, abs_act, form, target_key, user, pw):
        # Do we actually use target_key?
        self.logger.info(f"Fuzzing login for {form}")
        # We need a dict of all keys except user and pw, then we try various user pw values
        if base_url.endswith("/"):
            base_url = base_url[:-1]

        # Construct url like http://domain:port without path and add destinaation
        url = urlparse(base_url)
        url = url.scheme + "://" + url.netloc
        if url.endswith("/"):
            url = url[:-1]
        if abs_act.startswith("/"):
            url = url + abs_act
        else:
            url = url + "/" + abs_act
        ...
        def value_for(param):
            if len([x for x in param['defaults'] if x is not None]):
                return param['defaults'][0]
            return ''

        base_params = {k: value_for(v) for (k,v) in form['params'].items() if k != user[0] and k != pw[0]}
        bad_auth = ("badusername", "badpassword")
        base_response = None

        for (user_guess, pw_guess) in [bad_auth] + default_auths:
            # Create a new param dict
            this_params = base_params.copy()
            this_params[user[0]] = user_guess
            this_params[pw[0]] = pw_guess
            self.logger.info(f"Trying to auto login to {url} with {form['method']} with {this_params}")
            # We want to try logging in, then grab cookies of each, and after analyazing results,
            # save the cookies for the best session and add to target.session
            this_session = requests.Session()

            try:
                self.active_request = (target, requests.Request(form['method'], url, data=this_params, headers={'Connection':'close'}), this_params)
                response = this_session.send(self.active_request[1].prepare(), timeout=TIMEOUT)
            finally:
                self.active_request = None
            ...
            print("Attempted login with", user, pw, "got:")
            print(response.text)

            if base_response:
                print("Compared to bad baseline:")
                print(base_response.text)
        
        # TODO: were any right? If so save them and their cookie?
        return False
