...
.. code-block:: bash
    ...
    cli_uprobes.py --sock results/latest/penguin_events.sock --path /lib/libc.so.6 --symbol malloc --action "print %d" --output /tmp/uprobe_output.txt
...
def uprobe(sock, path, symbol, action, proc, pid, output):
    """
