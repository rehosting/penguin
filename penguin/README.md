Penguin: Configurable rehosting with IGLOO
====
...
Given a configuration, Penguin uses IGLOO to rehost the system with the specified 
...
## From source
Clone the necessary dependencies and then build your container from source
```
./platform/dev/setup_repos.sh
./platform/dev/build_dev.sh
```
...
If you instead see a similar line with a non-zero exit code, this could indiciate
...
**const_map_file**: like const_map, but adds a `filename` field. Will create this file on your host (container) if it doesn't exist with contents based on the const_map details and then read from the host file.
