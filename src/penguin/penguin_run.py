...
def run_config(
    ...
    # Process-per-boot reboot support. allow_reboot: a guest reboot terminates
    # this process; the manager respawns the next boot. reboot_generation is 0
    # for the first boot of a series and incremented by the manager for each
    # respawn (via PENGUIN_REBOOT_GENERATION).
    allow_reboot = conf["core"].get("allow_reboot", False)
    reboot_generation = int(os.environ.get("PENGUIN_REBOOT_GENERATION", "0") or "0")
    ...
    # Exception: on a reboot continuation (generation > 0) we keep the existing
    # out_dir so the host-backed /igloo/shared mount and prior boot artifacts
    # survive into the next boot of the same series.
    if os.path.isdir(out_dir) and reboot_generation == 0:
    ...
    # Always pass -no-reboot: a guest-initiated reboot terminates this QEMU
    # process rather than resetting in place. With core.allow_reboot=True the
    # manager (PandaRunner's respawn loop) starts a fresh process for the
    # next boot. Disk persistence across the respawn follows core.immutable:
    # with immutable=False the guest writes straight through to the base image
    # so the next boot sees them (real reboot semantics); with the default
    # immutable=True each boot is throwaway. The guest-reset is still reported
    # to us first (see the reset callback below) so we can exit with a distinct
    # "reboot" status.
    args += ["-no-reboot"]
    ...
    # Register reboot callback if allowed. QEMU calls qemu_system_reset_request()
    # (which invokes this callback as its first action) BEFORE -no-reboot turns
    # the reset into a process shutdown. So on a guest-initiated reboot we still
    # learn the cause here, record it, and let QEMU exit; the manager then
    # respawns a fresh process for the next boot (process-per-boot). We do NOT
    # tear down / reinit plugins in-process -- the fresh process does a normal
    # clean boot, which sidesteps host/guest transport re-sync and plugin
    # lifecycle idempotency problems entirely.
    #
    # ShutdownCause enum ordinal for a guest-initiated reset. The enum is
    # generated by QAPI from qemu/qapi/run-state.json; its order is:
    #   none=0, host-error=1, host-qmp-quit=2, host-qmp-system-reset=3,
    #   host-signal=4, host-ui=5, guest-shutdown=6, guest-reset=7,
    #   guest-panic=8, subsystem-reset=9, snapshot-load=10.
    SHUTDOWN_CAUSE_GUEST_RESET = 7
    _reboot_callback_handle = None
    # One-element box the callback flips; read after panda.run() returns.
    reboot_state = {"requested": False}
    if allow_reboot:

        def _on_guest_reset(reason, opaque):
            """Callback invoked by QEMU when the guest requests a reset.

            Runs in QEMU's main-loop thread. Keep it minimal: just record that a
            guest reboot happened so the process can exit with a distinct status.
            """
            if reason == SHUTDOWN_CAUSE_GUEST_RESET:
                logger.info(
                    "Guest-initiated reboot detected; exiting for manager-driven "
                    "respawn (process-per-boot)")
                reboot_state["requested"] = True

        try:
            # Create CFFI callback and register it with QEMU
            cb_type = "void(int, void *)"
            _reboot_callback_handle = panda.ffi.callback(cb_type)(_on_guest_reset)
            panda.lib.set_penguin_reset_request_callback(
                _reboot_callback_handle, panda.ffi.NULL
            )
            logger.debug("Registered guest reboot callback")
        except Exception as e:
            # Registration failure indicates a broken build (CFFI binding missing
            # or QEMU library incompatibility) — fail loudly rather than silently
            # skipping reboot handling.
            raise RuntimeError(
                f"Failed to register guest reboot callback: {e}. This indicates "
                "a broken QEMU build or missing CFFI bindings."
            ) from e
    ...
    # Tell the caller whether this boot ended in a guest-initiated reboot, so a
    # manager can respawn the next boot of the series (process-per-boot).
    return reboot_state["requested"]
...
def main():
    ...
    if rebooted:
        # Distinct status: the guest asked to reboot. The manager respawns
        # the next boot; a normal shutdown falls through to the default 0 exit.
