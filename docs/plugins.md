...
The plugin also writes two convenience files in the results directory,
regenerated as bridges are established: `guest_ip` (the bare reachable IP, same
content as `ipaddr.txt`) and `endpoints.txt` (one
`proto host_ip:host_port -> guest_ip:guest_port (proc)` line per bridge).

#### The published port is not always the guest's port

`host_port` matches `guest_port` when it can, but the plugin has to move it when
that port is unavailable — already bound in the container, already claimed by an
earlier bind in this same run, or privileged (below 1024) when the process
cannot bind privileged ports. It then searches deterministically in strides of
1000: guest port 80 becomes 1080, or 2080 if 1080 is taken too, and so on.

So the same guest port can be published on different host ports in different
runs of the same container, without anything having changed in the config. That
is working as intended, but it means **nothing may hardcode a host port**. Read
it from `endpoints.txt` (or the `host_port` column of `netbinds.csv`), which is
the only source of truth for a given run. The run log also states the mapping
and the reason for it as each bind happens:

```
        lighttpd binds tcp 0.0.0.0:80    reach it at container foo:2080   80 is already in use
```

An explicit mapping is available when you need a fixed port: set
`plugins.vpn.IGLOO_VPN_PORT_MAPS` (or the `IGLOO_VPN_PORT_MAPS` environment
variable) to comma-separated `<proto>:<host_port>:<guest_ip>:<guest_port>`
rules, e.g. `tcp:8080:192.168.0.1:80`. A fixed mapping is never relocated: if
the host port is busy the run fails loudly instead.

### connect.sh

Every run also gets an executable `results/<n>/connect.sh`, generated by
penguin (not the VPN plugin) next to `runtime.yaml`. It is the one-glance "get
me into this device" helper and talks to the guest's **serial root shell**
through the container's localhost
(`docker exec <container> telnet localhost <port>`), so it does **not** depend
