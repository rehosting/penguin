...
If False (default), a guest-initiated reboot terminates the emulation (one boot per run). If True, a guest reboot instead restarts the guest: the current QEMU process exits and the manager launches a fresh process for the next boot (process-per-boot). Whether the guest's disk writes survive the reboot follows core.immutable (immutable=False persists them, the default immutable=True does not). Each boot is a clean boot, so plugins keep their 'one boot per run' assumptions.
