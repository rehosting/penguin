...
the forcing function: bump ``IGLOO_DRIVER_VERSION`` (Dockerfile) to a release
...
def test_pinned_driver_has_do_exit_hook(tmp_path, igloo_ko_isf):
    ...
    assert hasattr(consts.HYPER_OP, "HYPER_OP_REGISTER_EXIT_HOOK"), (
        ...
        "Bump IGLOO_DRIVER_VERSION (Dockerfile) to a release carrying the hook.")
    assert hasattr(consts.igloo_hypercall_constants, "IGLOO_HYP_PROC_EXIT"), (
        "pinned igloo_driver ISF lacks IGLOO_HYP_PROC_EXIT: bump "
        "IGLOO_DRIVER_VERSION to a release carrying the do_exit hook.")
    ...
    try:
        ...
        size = 0
    assert size > 0, (
        "pinned igloo_driver ISF lacks struct exit_event: bump "
        "IGLOO_DRIVER_VERSION to a release carrying the do_exit hook.")
