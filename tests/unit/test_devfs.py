...
so we load it with ``real_isf=`` — the real driver ISF via ``dwarffi`` — so
``hyper.consts`` builds with real enum values. This is the kernel-facing half of
the pseudofile stack: ``register_devfs`` queues a composed DevFile for portal
registration, and ``_get_or_create_dev_dir`` actually emits the portal command.

We verify the **portal command logic**: the right op is chosen
(``HYPER_OP_DEVFS_CREATE_OR_LOOKUP_DIR``, carrying its real op number), the request
bytes are carried as the command payload, and the portal's returned id is threaded
back and cached.
"""
...
class _KFFI(RealKffi):
    ...
    ``new`` returns a fixed request blob so bytes()/len() work."""
...
def _load(tmp_path, isf):
    return load_pyplugin(DEVFS, outdir=tmp_path, real_isf=isf,
                         doubles={"kffi": _KFFI([isf])})


# --- register_devfs: the queue/dedup handoff -------------------------------- #
def test_register_devfs_queues_file_and_interrupt(tmp_path, igloo_ko_isf):
...
def test_register_devfs_defaults_major_minor_from_file(tmp_path, igloo_ko_isf):
    lp = _load(tmp_path, igloo_ko_isf)
...
def test_register_devfs_rejects_duplicate(tmp_path, igloo_ko_isf):
    lp = _load(tmp_path, igloo_ko_isf)
...
def test_register_devfs_requires_a_path(tmp_path, igloo_ko_isf):
    lp = _load(tmp_path, igloo_ko_isf)
    with pytest.raises(ValueError):
...
def test_dir_creation_emits_correct_portal_command(tmp_path, igloo_ko_isf):
    lp = _load(tmp_path, igloo_ko_isf)
