...
class VtypeBaseType:
    ...
    def get_compiled_struct(self) -> Optional[struct.Struct]:
        if hasattr(self, '_compiled_struct') and self._compiled_struct is not None: 
            if self.size == 0 and self._compiled_struct is None : return None 
        ...
        if self.kind == "int" or self.kind == "pointer":
            if self.size == 1: fmt_char = 'b' if self.signed else 'B'
            elif self.size == 2: fmt_char = 'h' if self.signed else 'H'
            elif self.size == 4: fmt_char = 'i' if self.signed else 'I'
            elif self.size == 8: fmt_char = 'q' if self.signed else 'Q'
        elif self.kind == "char":
            if self.size == 1: fmt_char = 'b' if self.signed else 'B' 
        elif self.kind == "bool":
            if self.size == 1: fmt_char = '?'
        elif self.kind == "float":
            if self.size == 4: fmt_char = 'f'
            elif self.size == 8: fmt_char = 'd'
...
class VtypeEnum:
    ...
    def get_name_for_value(self, value: int) -> Optional[str]:
        if self._val_to_name is None: self._val_to_name = {v: k for k, v in self.constants.items()}
...
class VtypeSymbol:
    ...
    def get_decoded_constant_data(self) -> Optional[bytes]:
        if self.constant_data:
            try: return base64.b64decode(self.constant_data)
...
class BoundTypeInstance:
    ...
    def __init__(self, type_name: str, type_def: Union[VtypeUserType, VtypeBaseType, VtypeEnum],
        ...
        if not isinstance(buffer, bytearray): # Should have been handled by VtypeJson.create_instance
    ...
    def _read_data(self, field_type_info: Dict[str, Any], field_offset_in_struct: int, field_name_for_error: str) -> Any:
        ...
        if kind == "base":
            if name is None: raise ValueError(f"Base type for field '{field_name_for_error}' has no name.")
            base_type_def = self._instance_vtype_accessor.get_base_type(name)
            if base_type_def is None: raise ValueError(f"Base type '{name}' not found for field '{field_name_for_error}'.")
            
            compiled_struct_obj = base_type_def.get_compiled_struct()
            if base_type_def.size == 0: return None 
            if compiled_struct_obj is None:
                 raise ValueError(f"Cannot get compiled struct for base type '{name}' (size: {base_type_def.size}).")

            try: 
                return compiled_struct_obj.unpack_from(self._instance_buffer, absolute_field_offset)[0]
            except struct.error as e: raise struct.error(f"Error unpacking base type '{name}' for field '{field_name_for_error}' at offset {absolute_field_offset} (buffer len {len(self._instance_buffer)}): {e}")
        
        elif kind == "pointer":
            ptr_base_type = self._instance_vtype_accessor.get_base_type("pointer")
            if ptr_base_type is None: raise ValueError("Base type 'pointer' definition not found.")
            
            compiled_struct_obj = ptr_base_type.get_compiled_struct()
            if compiled_struct_obj is None: 
                raise ValueError("Cannot get compiled struct for 'pointer' base type.")
            try:
            ...
            except struct.error as e: raise struct.error(f"Error unpacking pointer for field '{field_name_for_error}' at offset {absolute_field_offset}: {e}")

        elif kind == "array":
            count = field_type_info.get("count", 0)
            subtype_info = field_type_info.get("subtype")
            if subtype_info is None: raise ValueError(f"Array field '{field_name_for_error}' has no subtype.")
            elements = []
            current_element_struct_offset = field_offset_in_struct
            for i in range(count):
                try:
                    element = self._read_data(subtype_info, current_element_struct_offset, f"{field_name_for_error}[{i}]")
                    elements.append(element)
                except (ValueError, struct.error) as e:
                    elements.append(f"<Error reading element {i} of {field_name_for_error}: {e}>")
                element_size = self._instance_vtype_accessor.get_type_size(subtype_info)
                if element_size is None:
                    if i < count - 1: elements.append(f"<Error: Cannot get subtype size for '{field_name_for_error}', array incomplete.>")
                    break
                current_element_struct_offset += element_size
            return elements

        elif kind == "struct" or kind == "union":
            if name is None: raise ValueError(f"User type for field '{field_name_for_error}' has no name.")
            user_type_def = self._instance_vtype_accessor.get_user_type(name)
            if user_type_def is None: raise ValueError(f"User type '{name}' not found for field '{field_name_for_error}'.")
            return BoundTypeInstance(name, user_type_def, self._instance_buffer, self._instance_vtype_accessor, absolute_field_offset)

        elif kind == "enum":
            if name is None: raise ValueError(f"Enum type for field '{field_name_for_error}' has no name.")
            enum_def = self._instance_vtype_accessor.get_enum(name)
            if enum_def is None: raise ValueError(f"Enum type '{name}' not found for field '{field_name_for_error}'.")
            if enum_def.base is None: raise ValueError(f"Enum '{name}' has no base type.")
            base_type_def = self._instance_vtype_accessor.get_base_type(enum_def.base)
            if base_type_def is None: raise ValueError(f"Base type '{enum_def.base}' for enum '{name}' not found.")
            ...
            if None in [bit_length, bit_position, underlying_type_info]: raise ValueError(f"Bitfield '{field_name_for_error}' missing properties.")
            underlying_base_name = underlying_type_info.get("name")
            if underlying_base_name is None: raise ValueError(f"Bitfield '{field_name_for_error}' underlying type has no name.")
            ...
            if underlying_base_def is None or underlying_base_def.size is None : 
                 raise ValueError(f"Cannot get underlying type definition or size for bitfield '{field_name_for_error}'.")
            
            compiled_struct_obj = underlying_base_def.get_compiled_struct()
            if compiled_struct_obj is None:
                 raise ValueError(f"Cannot get compiled struct for bitfield '{field_name_for_error}' underlying type.")
        ...
        elif kind == "function": return f"<FunctionType: {field_type_info.get('name', 'anon_func')}>"
        elif kind == "void" and field_type_info.get("name") == "void": return None
        ...
        if kind == "base":
            ...
            if base_type_def.size == 0: 
                if value_to_write is not None:
                    raise ValueError(f"Cannot write value to void type field '{field_name_for_error}'.")
                return 
            if compiled_struct_obj is None:
                 raise ValueError(f"Cannot get compiled struct for base type '{name}' (size: {base_type_def.size}) to write field '{field_name_for_error}'.")
            try:
                compiled_struct_obj.pack_into(self._instance_buffer, absolute_field_offset, value_to_write)
            except struct.error as e:
                raise struct.error(f"Error packing base type '{name}' for field '{field_name_for_error}' at offset {absolute_field_offset} with value '{value_to_write}': {e}")

        elif kind == "pointer":
            ptr_base_type = self._instance_vtype_accessor.get_base_type("pointer")
            if ptr_base_type is None: raise ValueError("Base type 'pointer' definition not found for writing.")
            
            compiled_struct_obj = ptr_base_type.get_compiled_struct()
            if compiled_struct_obj is None: 
                raise ValueError("Cannot get compiled struct for 'pointer' base type for writing.")
            ...
            if isinstance(value_to_write, Ptr):
                ...
                raise TypeError(f"Cannot write type '{type(value_to_write)}' to pointer field '{field_name_for_error}'. Expected Ptr or int.")
            try:
                compiled_struct_obj.pack_into(self._instance_buffer, absolute_field_offset, address_to_write)
            except struct.error as e:
                raise struct.error(f"Error packing pointer for field '{field_name_for_error}' at offset {absolute_field_offset} with address {address_to_write:#x}: {e}")

        elif kind == "enum":
            if name is None: raise ValueError(f"Enum type for field '{field_name_for_error}' has no name.")
            enum_def = self._instance_vtype_accessor.get_enum(name)
            if enum_def is None: raise ValueError(f"Enum type '{name}' not found for field '{field_name_for_error}'.")
            if enum_def.base is None: raise ValueError(f"Enum '{name}' has no base type defined for writing.")
            base_type_def = self._instance_vtype_accessor.get_base_type(enum_def.base)
            if base_type_def is None: raise ValueError(f"Base type '{enum_def.base}' for enum '{name}' not found for writing.")
            ...
            if isinstance(value_to_write, EnumInstance):
                ...
                if found_val is None:
                    raise ValueError(f"Enum constant name '{value_to_write}' not found in enum '{name}'.")
                int_val_to_write = found_val
            else:
                raise TypeError(f"Cannot write type '{type(value_to_write)}' to enum field '{field_name_for_error}'. Expected EnumInstance, int, or str.")
            try:
                compiled_struct_obj.pack_into(self._instance_buffer, absolute_field_offset, int_val_to_write)
            except struct.error as e:
                raise struct.error(f"Error packing enum '{name}' for field '{field_name_for_error}' at offset {absolute_field_offset} with value {int_val_to_write}: {e}")
        
        elif kind == "bitfield":
            bit_length = field_type_info.get("bit_length")
            bit_position = field_type_info.get("bit_position")
            underlying_type_info = field_type_info.get("type")
            if None in [bit_length, bit_position, underlying_type_info]: raise ValueError(f"Bitfield '{field_name_for_error}' missing properties for writing.")
            
            underlying_base_name = underlying_type_info.get("name")
            if underlying_base_name is None: raise ValueError(f"Bitfield '{field_name_for_error}' underlying type has no name for writing.")
            underlying_base_def = self._instance_vtype_accessor.get_base_type(underlying_base_name)
            if underlying_base_def is None or underlying_base_def.size is None: 
                 raise ValueError(f"Cannot get underlying type definition or size for bitfield '{field_name_for_error}' for writing.")
            
            compiled_struct_obj = underlying_base_def.get_compiled_struct()
            if compiled_struct_obj is None:
                 raise ValueError(f"Cannot get compiled struct for bitfield '{field_name_for_error}' underlying type for writing.")
            ...
            if not isinstance(value_to_write, int):
                raise TypeError(f"Value for bitfield '{field_name_for_error}' must be an integer, got {type(value_to_write)}.")
            ...
            try:
                compiled_struct_obj.pack_into(self._instance_buffer, absolute_field_offset, new_storage_val)
                 ...
                 raise struct.error(f"Error packing bitfield '{field_name_for_error}' at offset {absolute_field_offset} with new storage value {new_storage_val}: {e}")

        elif kind == "array" or kind == "struct" or kind == "union":
            raise NotImplementedError(f"Direct assignment to field '{field_name_for_error}' of type '{kind}' is not supported. Modify elements or nested fields individually.")
        
        else: 
            raise TypeError(f"Cannot write to field '{field_name_for_error}' of unhandled type kind '{kind}'.")


    def __getattr__(self, name: str) -> Any:
        if name.startswith('_instance_'): return super().__getattribute__(name)
        if name in self._instance_cache: return self._instance_cache[name]
        if not isinstance(self._instance_type_def, VtypeUserType):
            raise AttributeError(f"Type '{self._instance_type_name}' is not struct/union, no field '{name}'.")
        field_def = self._instance_type_def.fields.get(name)
        if field_def is None: raise AttributeError(f"'{self._instance_type_name}' has no attribute '{name}'")
        if field_def.offset is None: raise ValueError(f"Field '{name}' in '{self._instance_type_name}' has no offset.")
        try:
            value = self._read_data(field_def.type_info, field_def.offset, name)
            if field_def.type_info.get("kind") in ["struct", "union", "array"]: self._instance_cache[name] = value
            return value
        except (struct.error, ValueError) as e: raise AttributeError(f"Error processing field '{name}': {e}")
    ...
    def __setattr__(self, name: str, value: Any):
        ...
        if not isinstance(self._instance_type_def, VtypeUserType):
            raise AttributeError(f"Cannot set attribute '{name}' on non-struct/union type '{self._instance_type_name}'.")
        ...
        if field_def.offset is None:
            raise ValueError(f"Field '{name}' in '{self._instance_type_name}' has no offset, cannot write.")

        try:
            self._write_data(field_def.type_info, field_def.offset, value, name)
    ...
    def to_bytes(self) -> bytes:
        ...
        """
        if not isinstance(self._instance_type_def, VtypeUserType):
            raise TypeError(f"to_bytes() is primarily for struct/union instances. Type is {self._instance_type_name}")

        size = self._instance_type_def.size
        if size is None:
            raise ValueError(f"Cannot determine size for type '{self._instance_type_name}' to get bytes.")
    ...
    def __dir__(self):
        ...
        if isinstance(self._instance_type_def, VtypeUserType): attrs.extend(self._instance_type_def.fields.keys())
...
class Ptr:
    ...
    @property
    def points_to_type_info(self) -> Optional[Dict[str, Any]]: return self._subtype_info
    @property
    def points_to_type_name(self) -> str:
        if not self._subtype_info: return "void"
...
class EnumInstance:
    ...
    def __init__(self, enum_def: VtypeEnum, value: int):
        ...
        self.value = value
    @property
    def name(self) -> Optional[str]: return self._enum_def.get_name_for_value(self.value)
    def __repr__(self) -> str:
    ...
    def __eq__(self, other):
        if isinstance(other, EnumInstance): return self.value == other.value and self._enum_def.name == other._enum_def.name
...
class VtypeJson:
    ...
    def get_base_type(self, name: str) -> Optional[VtypeBaseType]:
        if name in self._parsed_base_types_cache: return self._parsed_base_types_cache[name]
    ...
    def get_user_type(self, name: str) -> Optional[VtypeUserType]:
        if name in self._parsed_user_types_cache: return self._parsed_user_types_cache[name]
    ...
    def get_enum(self, name: str) -> Optional[VtypeEnum]:
        if name in self._parsed_enums_cache: return self._parsed_enums_cache[name]
    ...
    def get_symbol(self, name: str) -> Optional[VtypeSymbol]:
        if name in self._parsed_symbols_cache: return self._parsed_symbols_cache[name]
    ...
    def create_instance(self, type_input: Union[str, VtypeUserType], 
                        buffer: Union[bytes, bytearray], # Accept bytes or bytearray
        ...
        if isinstance(type_input, str):
            ...
            raise TypeError(f"type_input must be a string (type name) or VtypeUserType object, got {type(type_input)}")
        ...
        if user_type_def:
            ...
            if user_type_def.size is not None and user_type_def.size > effective_len:
                raise ValueError(f"Buffer too small for '{type_name_for_instance}' at offset {instance_offset_in_buffer}. Need {user_type_def.size}, got {effective_len}.")
        ...
        if isinstance(type_input, str):
            if self.get_base_type(type_input): 
                raise NotImplementedError(f"Direct instance creation for base type '{type_input}' not primary use case. Use for struct/union types.")
            if self.get_enum(type_input): 
                raise NotImplementedError(f"Direct instance creation for enum type '{type_input}' not primary use case. Use for struct/union types.")
        
        raise ValueError(f"User type definition for '{type_input if isinstance(type_input, str) else type_input.name}' not found or not a VtypeUserType.")
...
def load_isf_json(json_input: Union[str, object]) -> VtypeJson:
    ...
    if input_is_path_str:
        ...
        try:
            ...
            raise FileNotFoundError(f"The ISF JSON file was not found: {path_str}")
    ...
    try:
        ...
        if args.find_symbol_at is not None:
            ...
            if symbols_at_addr:
                ...
                for sym_obj in symbols_at_addr: print(f"    - {sym_obj}")
            else: print(f"  No symbols found at address {args.find_symbol_at:#x}.")
            if isf_data._address_to_symbol_list_cache is not None:
                 print(f"  Address-to-symbol cache is now populated with {len(isf_data._address_to_symbol_list_cache)} entries.")

        if args.verbose:
            print("\n--- Verbose Information ---")
            print(f"  Metadata Producer: {isf_data.metadata.producer.get('name', 'N/A')}, Version: {isf_data.metadata.producer.get('version', 'N/A')}")
        ...
        if args.test_write or args.test_to_bytes:
            ...
            my_struct_def = isf_data.get_user_type("my_struct") # Assumes 'my_struct' exists for test
            if my_struct_def and my_struct_def.size is not None:
                ...
                struct.pack_into("<B", initial_bytes_data, 4, 0b00000001) # status_flags=1, type_flag=0
                # ... (initialize other fields if necessary for a complete test)
                
                # Pass the bytearray to create_instance
                instance = isf_data.create_instance("my_struct", initial_bytes_data) 
                
                if args.test_write:
                    print(f"  Initial id: {instance.id}")
                    instance.id = 999
                    print(f"  Modified id: {instance.id} (Buffer check: {struct.unpack_from('<i', initial_bytes_data, 0)[0]})") # Check original bytearray
                    # ... (more write tests as before) ...

                if args.test_to_bytes:
                    instance_bytes = instance.to_bytes()
                    print(f"  instance.to_bytes() (hex): {instance_bytes.hex()}")
                    # Verify the slice is correct
                    expected_bytes = bytes(initial_bytes_data[instance._instance_offset : instance._instance_offset + my_struct_def.size])
                    if instance_bytes == expected_bytes:
                        print("  to_bytes() content matches expected slice of the buffer.")
                    else:
                        print("  ERROR: to_bytes() content MISMATCH!")
                        print(f"    Expected (hex): {expected_bytes.hex()}")
            else:
                print("  Skipping write/to_bytes test: 'my_struct' not found or has no size in the loaded ISF.")
