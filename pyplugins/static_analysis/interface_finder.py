...
class InterfaceFinder(StaticAnalysisPlugin):
    """
    Identify network interfaces in the filesystem.
    """
    def run(self) -> dict[str, list[str]] | None:
        from .file_system_helper import FileSystemHelper

        pattern = re.compile(r"/sys/class/net/([a-zA-Z0-9_]+)", re.MULTILINE)
        sys_net_ifaces = FileSystemHelper.find_regex(pattern, self.extracted_fs).keys()
        ...
        for p in patterns:
            interfaces.update(FileSystemHelper.find_regex(p, self.extracted_fs).keys())
