...
class PseudofileFinder(StaticAnalysisPlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Load resources... For static plugins we'll need to figure out the right path.
        # Assuming penguin module's resources dir:
        import penguin
        resources = os.path.join(os.path.dirname(penguin.__file__), "resources")
        proc_sys_txt = os.path.join(resources, "proc_sys.txt")
        if os.path.exists(proc_sys_txt):
            with open(proc_sys_txt, "r") as f:
    ...
    def run(self) -> dict[str, list[str]]:
        ...
        dev_files = self._filter_files(
            self.extracted_fs, dev_pattern, [], self.IGLOO_ADDED_DEVICES
        )

        proc_files = self._filter_files(
            self.extracted_fs, proc_pattern, self.PROC_IGNORE, self.IGLOO_PROCFS
        )
