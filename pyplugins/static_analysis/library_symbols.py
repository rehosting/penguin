...
class LibrarySymbols(StaticAnalysisPlugin):
    """
    ...
    def run(self) -> dict[str, dict]:
        self.archend = arch_end(self.prior_results.get('ArchId'))
        ...
        for root, _, files in os.walk(self.extracted_fs):
            for file in files:
                ...
                if file_path.is_file() and \
                    ...
                    try:
                        found_nvram, found_syms = self._analyze_library(str(file_path),
                                                                        self.archend)
                        ...
                        continue
                    tmpless_path = str(file_path).replace(str(self.extracted_fs), "")
