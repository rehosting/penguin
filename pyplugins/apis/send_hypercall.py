...
class SendHypercall(Plugin):
    ...
    def on_send_hypercall(self, cpu: Any, buf_addr: int,
        ...
        if cb is None:
            # Guest-side init scripts fire some hypercalls (e.g. readiness,
            # core_pattern_lock) best-effort: a config generated by an older
            # penguin may not load the plugin that subscribes to them. That is
            # benign, so warn once per command instead of erroring every boot.
            if cmd not in self._unhandled_warned:
                self._unhandled_warned.add(cmd)
                self.logger.warning(
                    f"No subscriber for send_hypercall command {cmd!r}; "
                    f"if this project predates the plugin that handles it, "
                    f"add that plugin to the config or regenerate the config"
                )
            return
