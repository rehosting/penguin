...
class Symbols(Plugin):
    ...
    def __init__(self):
        ...
        if not self.nm_path:
            self.logger.warning("Host 'nm' utility not found. Symbol lookup will be slower.")
    ...
    def find_all(self, symbol: str) -> List[Tuple[str, int]]:
        ...
        for lib_path, lib_symbols in db.items():
            offset = self._resolve_symbol_in_dict(lib_symbols, symbol, demangled_target)
    ...
    def _load_symbols_db(self) -> Dict[str, Any]:
        ...
        if os.path.exists(self.libsymbols_path):
            try:
                ...
                self.logger.info(f"Loaded symbols DB from {self.libsymbols_path}")
        ...
        if '*' in path:
            ...
            for lib_path, lib_symbols in db.items():
                if pattern in lib_path or pattern in os.path.basename(lib_path):
                    offset = self._resolve_symbol_in_dict(lib_symbols, symbol, demangled_target)
                    if offset is not None: return lib_path, offset
            return None, None
        ...
        if demangled_target:
            for name, offset in symbols_dict.items():
                if name.startswith('_Z'):
                    try:
                        if cxxfilt.demangle(name) == demangled_target: return offset
                    except: pass
        return None
    ...
    def _scan_nm(self, path: str, symbol: str) -> Tuple[Optional[str], Optional[int]]:
        ...
        try:
            ...
            proc.terminate()
            if hasattr(proc, 'wait'): proc.wait()
            ...
            if new_symbols:
                with open(host_path, 'rb') as tf:
                    ...
                    for s_name, s_vaddr in new_symbols.items():
                        offset = self._vaddr_to_file_offset_optimized(segments, s_vaddr)
                        if offset is not None:
                    ...
                    if isinstance(section, SymbolTableSection):
                        for sym in section.iter_symbols():
                            if sym['st_shndx'] != 'SHN_UNDEF':
                                s_name = sym.name
                                offset = self._vaddr_to_file_offset_optimized(segments, sym['st_value'])
                                if offset is not None:
    ...
    def _resolve_staticfs_symlink(self, path: str) -> str:
        ...
        while True:
            if current_path in visited: return path 
            visited.add(current_path)

            try:
                mount_source = getattr(fs, "_fs", None)
                if not mount_source: return current_path
                
                file_info = mount_source.lookup(current_path)
                if not file_info: return current_path
