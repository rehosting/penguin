...
def _read_fail_hint(fname: str) -> str:
    if _is_synthetic(fname):
        return (f" -- {fname} is on a synthetic (VFS) filesystem whose files are "
                ...
                "~5.10 some of these paths (/proc/<pid>/*, /proc/net/*, and "
                "EVERY penguin-modelled pseudofile, because no read mixin "
                "defines read_iter -- see "
                "tests/unit/test_pseudofile_host_readability.py) are refused by "
                "__kernel_read for "
