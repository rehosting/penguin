...
deliveries after the kernel has selected a target. Default-fatal signals without
a userspace handler may start process or thread-group exit before that delivery
hook is reached. For process-preserving behavior, Lifeguard still suppresses
signal-send syscalls before the kernel applies signal semantics. SIGKILL and
SIGSTOP cannot be caught or ignored by Linux processes, so Lifeguard only
suppresses instances generated by supported signal-send syscalls. Synchronous
fault signals such as SIGILL or SIGSEGV may need guest state repair when
dropped; otherwise execution can return to the same faulting instruction.
"""
