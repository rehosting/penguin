...
class TaskSizeVerify(Plugin):
    ...
    def verify_task_size(self, regs, proto, syscall, fd, op, arg):
        ...
        self.logger.info(f"TaskSizeVerify: Expected={expected:#x}, Actual={actual:#x}")
        ...
        passed = True
        if actual != expected:
            self.logger.error(f"TaskSizeVerify: FAIL - actual task_size {actual:#x} != expected {expected:#x}")
            passed = False
        
        if max_addr > expected:
            self.logger.error(f"TaskSizeVerify: FAIL - mapping end {max_addr:#x} exceeds task_size {expected:#x}")
            passed = False
            for m in mappings:
                self.logger.info(f"Mapping: {m.start:#x} - {m.end:#x} {m.name}")
