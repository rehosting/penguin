...
class SeekableRWProcFile(ProcFile):
    ...
    def lseek(self, ptregs: PtRegsWrapper, file: FilePtr, offset: LoffT, whence: CInt):
        ...
        self.logger.debug(
            f"lseek called with offset={offset_val}, whence={whence_val}")
...
class ReleaseTrackingProcFile(ProcFile):
    ...
    def open(self, ptregs: PtRegsWrapper, inode: InodePtr, file: FilePtr):
        ...
        self.total_opens += 1
        self.logger.info(
            f"release_tracker opened. Active: {self.active_opens}")
        ptregs.retval = 0
...
class ProcTest(Plugin):
    def __init__(self):
        ...
        try:
            ...
            self.logger.info(
                "Successfully caught duplicate proc registration.")
