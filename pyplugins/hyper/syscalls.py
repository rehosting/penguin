...
class Syscalls(PyPlugin):
    ...
    def __init__(self, panda):
        ...
        self.portal.register_interrupt_handler("syscalls", self._syscall_interrupt_handler)
        
        # Register handlers for syscall setup and events
        self.panda.hypercall(IGLOO_HYP_SETUP_SYSCALL)(self._setup_syscall_handler)
        
        # Register syscall enter/return hypercalls
        self.panda.hypercall(IGLOO_HYP_SYSCALL_ENTER)(self._syscall_enter_event)
        self.panda.hypercall(IGLOO_HYP_SYSCALL_RETURN)(self._syscall_return_event)
    ...
    def _setup_syscall_handler(self, cpu):
        ...
        try:
            ...
            if sysinfo.syscall_nr == -1:
                self.logger.debug(f"Syscall {sysinfo.name} not registered, saving for later")
                self.missing_syscalls.append(sysinfo)
            else:
                self.syscall_info_table[sysinfo.syscall_nr] = sysinfo
                self.logger.debug(f"Registered syscall {sysinfo.name} ({sysinfo.syscall_nr})")
    ...
    def _get_proto(self, cpu, sce):
        if sce.nr in self.syscall_info_table:
            ...
            name = self.panda.read_str(cpu, sce.syscall_name.address).lstrip('_')
            scname = f'sys_{name}'
            if n := self.find_missing_syscall(scname):
                ...
                self.logger.debug(f"Syscall {sce.nr} ({sce.name}) not registered, using generic prototype")
        return proto
    ...
    def register_syscall_hook(self, hook_config):
        ...
        if hook_config.get("procname", None):
            ...
            procname = hook_config.get("procname", "") or ""  # Ensure procname is never None
