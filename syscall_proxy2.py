...
class MySingleRequestReverseProxy(proxy.ReverseProxyResource):
    ...
    def __init__(self, panda, listen_port, host, port, path, reactor=reactor):
    ...
    def getChild(self, path, request):
        return MySingleRequestReverseProxy(
            self.panda, self.listen_port, self.host, self.port, self.path + b'/' + path)
...
def run_proxy(panda, local_host, local_port, remote_host, remote_port):
    ...
    site = server.Site(MySingleRequestReverseProxy(panda, local_port, local_host, remote_port, b''))
