...
# IDEA: provide on_init/on_reset functions
# so crawler can reset/re-init guest by passing panda object

def on_init(panda):
    '''
    Runs at first sycall
    '''
    panda.load_plugin("osi", {"disable-autoload": True})
    panda.load_plugin("osi_linux",
        {'kconf_file': os.path.dirname(os.path.realpath(__file__))+'/virt.conf',
...
def on_reset(panda):
    panda.unload_plugin("osi_linux")
    panda.unload_plugin("osi")
